...
function useToggle({
  ...
  const wasControlled = React.useRef(onIsControlled).current
  React.useEffect(() => {
    warning(
      !(wasControlled !== onIsControlled),
      'changing from controlled to uncontrolled or v.v.',
    )
  }, [onIsControlled, wasControlled])

  React.useEffect(() => {
    const withOnChange = onChange !== null && onChange !== undefined
    if (onIsControlled && !withOnChange && !readOnly) {
      console.error(
